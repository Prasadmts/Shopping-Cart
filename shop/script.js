...
//storing items to their respective category
let men = []; 
let women = []; 
let jewelery = [];
let electronics = []; 
...
//allFilter function(starting function)
...
function showAll() {
  ...
  const men_html = men.map((item) => {
    return renderItems(item);
  });
  document.getElementById("mens-items").innerHTML = men_html.join("");

  const women_html = women.map((item) => {
    return renderItems(item);
  });
  document.getElementById("womens-items").innerHTML = women_html.join("");

  const jewel_html = jewelery.map((item) => {
    return renderItems(item);
  });
  document.getElementById("jewellery-items").innerHTML = jewel_html.join("");

  const elec_html = electronics.map((item) => {
    return renderItems(item);
  });
  document.getElementById("electronics-items").innerHTML = elec_html.join("");
}
